...
const VERSION = 1;
const EXCLUDED_FIELD = /(password|secret|token|credential|passkey|file|receipt|confirm)/i;
...
function removeDraftsOutsideScope(scope: string | null): void {
    ...
    try {
        ...
        const currentPrefix = scope ? PREFIX + encodeURIComponent(scope) + ':' : null;
        for (let index = storage.length - 1; index >= 0; index -= 1) {
            const key = storage.key(index);
            if (key?.startsWith(PREFIX) && (!currentPrefix || !key.startsWith(currentPrefix))) storage.removeItem(key);
        }
...
function sanitizeValues<Values extends Record<string, unknown>>(values: Values): Values {
    const sanitize = (value: unknown, key = ''): unknown => {
        if (EXCLUDED_FIELD.test(key) || typeof File !== 'undefined' && value instanceof File || typeof Blob !== 'undefined' && value instanceof Blob) return undefined;
...
function buildKey(scope: string, formKey: string, recordKey: string | number): string {
    return PREFIX + encodeURIComponent(scope) + ':' + encodeURIComponent(formKey) + ':' + encodeURIComponent(String(recordKey));
}

function readDraft<Values extends Record<string, unknown>>(key: string): DraftEnvelope<Values> | null {
    if (!storageAvailable()) return null;
    try {
        const value = window.sessionStorage.getItem(key);
        if (!value) return null;
        const draft = JSON.parse(value) as DraftEnvelope<Values>;
        if (draft.version !== VERSION || !draft.values || typeof draft.values !== 'object') {
...
export function useSessionDraft<Values extends FormDataType<Values> & Record<string, unknown>>({
    scope,
    formKey,
    recordKey,
    initialValues,
}: Options<Values>) {
    const storageKey = useMemo(() => scope ? buildKey(scope, formKey, recordKey) : null, [scope, formKey, recordKey]);
    ...
    useEffect(() => {
        if (draftEnvelope) {
            form.setData({ ...initialValuesRef.current, ...draftEnvelope.values });
    ...
    useEffect(() => {
        if (!initialized || !storageKey || !storageAvailable() || !form.isDirty || form.processing) return;
    ...
    const submit = useCallback(
        (method: 'post' | 'put' | 'patch' | 'delete', url: string, options: UseFormSubmitOptions = {}) => {
            ...
            form.submit(method, url, {
                ...
                onNetworkError: (error) => {
                    setConnectionError('Não foi possível conectar. Seus dados foram mantidos para nova tentativa.');
    ...
    return { form, draftRestored, connectionError, saveDraft, clearDraft, discardDraft, submit };
}

export function clearDraftSessionOnNavigation(scope: string | null | undefined): void {
    removeDraftsOutsideScope(scope ?? null);
}
